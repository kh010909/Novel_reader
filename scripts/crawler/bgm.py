...
#   return output
import requests
API_URL = "https://api-inference.huggingface.co/models/nanaaaa/emotion_chinese_english"
headers = {"Authorization": "Bearer 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"}
...
def analyze_emotion(text):
  ...
  #print(result)
  try:
    return Emotion[result[0][0]['label']]
  except:
     pass
  return 1
...
def add_music(text):
  ...
  for text in text_list:
    ...
    emotion = analyze_emotion(text)
    #add music tag to the start of the article, or return music data(let the function outside do this work)
    div = '<div id="music-{}" class=" memo-{}">'.format(num, emotion)
    text = div + text_ori + "</div>"
    ##whole_music_type.append(novel_type)
    whole_music_emotion.append(emotion)
    output = output + text
  ...
  div = '<div id="music-{}" class=" memo-{}">'.format("whole", page_emo)
  output = div + output + "</div>"
